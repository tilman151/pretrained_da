...
def run(source, target, broken, domain_tradeoff, record_embeddings, pretraining_reps, replications, gpu):
    pretrained_checkpoints = run_pretraining(source, target, broken, [0.01], record_embeddings, pretraining_reps, gpu)
    for b, checkpoint_per_tradeoff in pretrained_checkpoints.items():
        for _, checkpoints in checkpoint_per_tradeoff.items():
            for pretrained_checkpoint in checkpoints:
                run_daan(source, target, [b], domain_tradeoff, False,
